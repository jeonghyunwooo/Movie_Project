      ...
      <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
          <div class="modal-header">
            <h1 class="modal-title fs-5" id="staticBackdropLabel" style="color:red; margin-left: 5%; font-weight: bold;">영화를 소개합니다</h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="color:red; margin-right: 1.5%; font-weight: bold; font-size: 20px; /* 작은 화면에 대한 텍스트 크기 */">X</button>
          </div>
          <div class="modal-body">
            ...
            <div class = 'cards'>
              <div class="card text-bg-dark">
                <svg class="bd-placeholder-img bd-placeholder-img-lg  card-img" width="100%" height="270" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Placeholder: Card image" preserveAspectRatio="xMidYMid slice" focusable="false">
                  <title></title>
                  <rect width="100%" height="100%" fill="#868e96"></rect><text x="50%" y="50%" fill="#dee2e6" dy=".3em"></text></svg>
              
                <div class="card-img-overlay">
                  <h5 class="card-title">Card title</h5>
                  <p class="card-text">This is a wider card with little bit longer.</p>
...
export default {
  ...
  computed: {
    ...
    getGenres() {
      if (Array.isArray(this.movie_detail.genres)) {
        const filteredGenres = this.movie_detail.genres.map(genre => genre.replace(/[^A-Za-z0-9가-힣\s]/g, ''))
        ...
        const filteredGenres = genresArray.map(genre => genre.replace(/[^A-Za-z0-9가-힣\s]/g, ''))
        return filteredGenres.filter(genre => genre !== '').join(', ')
      } else {
        return ''
      }
  ...
  methods: {
    checkOverflow() {
      ...
      const lineHeight = parseFloat(getComputedStyle(overviewElement).lineHeight);
...
.card-img {
  background-color: rgba(0, 0, 0, 1); /* 이미지의 배경색을 투명도를 갖는 색상으로 설정 */
}
</style>